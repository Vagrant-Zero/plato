...
func (tw *TimingWheel) addOrRun(t *Timer) {
	if !tw.add(t) {
		...
		// always execute the timer's task in its goroutine
		go t.task()
	}
}
...
type Scheduler interface {
	...
	// It will return a zero time if no next it scheduled.
	//
...
// Afterward, it will ask the next execution time each time f is about to
// be executed, and f will be called at the next execution time if the time
// is non-zero.
func (tw *TimingWheel) ScheduleFunc(s Scheduler, fn func()) (t *Timer) {
