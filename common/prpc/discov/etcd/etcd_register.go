...
func (r *Register) registerService(ctx context.Context, service *registerService) {
	...
	keepAliveChan, err := r.cli.KeepAlive(ctx, leaseGrantResp.ID)
	if err != nil {
		logger.CtxErrorf(ctx, "register service keepalive err:%v", err)
		return
	}

	service.keepAliveChan = keepAliveChan
...
func (r *Register) unRegisterService(ctx context.Context, service *discov.Service) {
	...
	if len(endpoints) == 0 {
		...
		// del service endpoints fail, put endpoints into map
...
func (r *Register) updateDownService(service *discov.Service) {
	...
	downService := r.downServices.Load().(map[string]*discov.Service)
	if _, ok := downService[service.Name]; !ok {
		downService[service.Name] = service
		r.downServices.Store(downService)
		return
	}

	for _, newAddEndpoint := range service.Endpoints {
		var isExist bool
		for idx, endpoint := range downService[service.Name].Endpoints {
			if newAddEndpoint.IP == endpoint.IP && newAddEndpoint.Port == endpoint.Port {
				downService[service.Name].Endpoints[idx] = newAddEndpoint
				isExist = true
				break
			}
		}
		...
		if !isExist {
			downService[service.Name].Endpoints = append(downService[service.Name].Endpoints, newAddEndpoint)
		}
	}
